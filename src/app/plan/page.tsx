...
import Skeleton from "react-loading-skeleton";
import "react-loading-skeleton/dist/skeleton.css";
import LogoutButton from "../logout/page"; // adjust path if needed
...
export default function PlanPage() {
 ...
 const parseItinerary = (text: string) => {
    ...
    return days.map((entry, index) => ({
      day: `Day ${index + 1}`,
      content: entry.replace(/Day \d+:/i, "").trim(),
    }));
  };

  const parsedItinerary = itinerary ? parseItinerary(itinerary) : [];
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-900 text-white p-6">
      <motion.section
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
        className="max-w-4xl mx-auto bg-white/10 backdrop-blur-md rounded-3xl shadow-2xl p-8 space-y-6"
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {parsedItinerary.map((day, idx) => (
                <motion.div
                  ...
                  className="bg-white/30 backdrop-blur-md p-6 rounded-2xl shadow-md text-white border border-white/20"
                >
